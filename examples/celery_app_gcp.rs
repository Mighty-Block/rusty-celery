...
use celery::{broker::GCPPubSubBroker, prelude::*};
use env_logger::Env;
...
async fn long_running_task(secs: Option<u64>) {
    time::sleep(Duration::from_secs(secs.unwrap_or(20))).await;
}
...
async fn main() -> Result<()> {
    ...
    let my_app = celery::app!(
        broker = GCPPubSubBroker { std::env::var("GCPPUBSUB_ADDR").unwrap_or_else(|_| "http://127.0.0.1:8538".into()) },
        tasks = [
            add,
            buggy_task,
            long_running_task,
